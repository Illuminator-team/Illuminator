...
class Compressor(ModelConstructor):
    parameters = {
        'compressor_type': 1,  # 0 = electrochemical' or 1 = 'mechanical'
        'p_in': 35,  # bar
        'p_out': 272,  # bar
        'T_amb': 293.15,  # K
        'mechanical_eff': 75.0,  # % - mechanical is more efficient
        'electrochemical_eff': 62.22,  # % - electrochemical is less efficient
        'nom_compression_energy': 8.5,  # kWh/kg (for electrochemical)
    }
    inputs = {
        'compressor_on': 0,
        'flow2c': 0,  # hydrogen flow to the compressor [kg/timestep]
        'phase_step': 0,  # phase step of electrolyser
        # 'tanks_soc': 50  # % SOC (needed for mechanical to calc tank pressure)
    }
    outputs = {
    ...
    mmh2 = 2.016  # g/mol
    R = 8.314  # J/mol*K
    gamma = 1.41  # for hydrogen

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.compressor_type = self._model.parameters.get('compressor_type')
        ...
        self.mechanical_eff = self._model.parameters.get('mechanical_eff')
        self.electrochemical_eff = self._model.parameters.get('electrochemical_eff')
        self.nom_compression_energy = self._model.parameters.get('nom_compression_energy')
    ...
    def step(self, time: int, inputs: dict = None, max_advance: int = 900) -> None:
        ...
        # tanks_soc = input_data['tanks_soc']

        # Calculate power based on compressor type
        if self.compressor_type == 1:
            power_data = self.power_req_mechanical(
                compressor_on=compressor_on,
                flow=flow,
                phase_step=phase_step,
                # tanks_soc=tanks_soc
            )
        elif self.compressor_type == 0:
            power_data = self.power_req_electrochemical(
                compressor_on=compressor_on,
                flow=flow,
                phase_step=phase_step
            )
    ...
    # def power_req_mechanical(self, compressor_on: int, phase_step: int, flow: float, tanks_soc: float) -> dict:
    def power_req_mechanical(self, compressor_on: int, phase_step: int, flow: float) -> dict:
        """
        Mechanical compression using isentropic equation.
        Power varies with tank pressure (tanks_soc determines current pressure).
        Uses mechanical efficiency (typically 70-80%).
        """
        if compressor_on == 0 and phase_step not in [2, 3, 4]:
            return {'power_req': 0, 'output_flow': 0}

        if flow == 0:
            return {'power_req': 0, 'output_flow': 0}

        # Calculate current tank pressure from SOC
        # current_tank_pressure = (tanks_soc / 100) * self.p_out
        
        # Compressor must output at least tank pressure to push hydrogen in
        # p_out_actual = max(current_tank_pressure + 1, self.p_out)
        p_out_actual = self.p_out
        
        pressure_ratio = p_out_actual / self.p_in

        # Isentropic work [J/mol]
        exponent = (self.gamma - 1) / self.gamma
        w_isentropic = (self.gamma / (self.gamma - 1)) * self.R * self.T_amb * (pressure_ratio ** exponent - 1)

        # Flow in mol/s
        flow_mol_s = (flow * 1000 / self.mmh2) / self.time_resolution

        # Real work accounting for mechanical efficiency
        w_real = w_isentropic / (self.mechanical_eff / 100)

        # Power in kW
        power_kw = -1*w_real * flow_mol_s / 1000

        return {'power_req': power_kw, 'output_flow': flow}

    def power_req_electrochemical(self, compressor_on: int, phase_step: int, flow: float) -> dict:
        """
        Electrochemical compression using fixed nominal energy per kg.
        Power independent of tank pressure (simplified model).
        Uses electrochemical efficiency (typically 60-70%).
        """
        if compressor_on == 0 and phase_step not in [2, 3, 4]:
            return {'power_req': 0, 'output_flow': 0}
        ...
        # Fixed compression energy per kg with electrochemical efficiency
        flow_kg_h = flow * (3600 / self.time_resolution)
        power_kw = -1*flow_kg_h * self.nom_compression_energy / (self.electrochemical_eff / 100)
    ...
    def update_outputs_and_states(self, flow: float, power_data: dict) -> None:
        ...
        volume_flow_out = (flow / density) / dt_h if flow > 0 else 0
    ...
    def find_z_val(self, press: float, temp: float) -> float:
        z_values = [
            [1.00070, 1.00004, 1.00060, 1.00055, 1.00047, 1.00041, 1.00041],
            ...
            [1.37284, 1.34392, 1.31784, 1.27398, 1.24173, 1.21583, 1.19463],
            [1.45188, 1.41618, 1.38797, 1.33010, 1.29040, 1.25920, 1.23373],
