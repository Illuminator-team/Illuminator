...
class EV(ModelConstructor):
    ...
    parameters={'end_initial_phase': 0,     # soc at which the mid-phase starts [%]
                ...
                'start_charge': 0}          # timestep index on which the car starts charging [-]
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.start_charge = self._model.parameters.get('start_charge')
    ...
    def step(self, time: int, inputs: dict=None, max_advance: int = 900) -> None:
        ...
        demand, self.soc = self.charge(load_in, max_advance)
        self.set_outputs({'demand': demand})
    ...
    def charge(self, available_power, max_advance):
        hours = max_advance/60/60
        ...
        if self.time >= self.start_charge and self.presence == 1 and self.soc < self.desired_soc:
