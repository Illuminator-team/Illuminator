...
#     inputs = {
#         'charge_battery': 0,  # 1 = charge, -1 = discharge, -2 = discharge for electrolyser, 0 = idle
#         'pv_battery': 0,          # kW
#         'load_battery': 0,        # kW
#         'flow2e': 0,
#         'p_out_fuelcell': 0                      # kW
...
#             signal=input_data['charge_battery'],
...
#     def output_power(self, signal: int, pv: float, load: float, flow2e: float, fc: float) -> dict:
#         soc = self.soc
#         cap_kwh = self.max_energy
#         flow2b = 0
#         bat_state = 'off'
#         dt_h= self.time_resolution / 3600

#         if signal == 1 and soc <= self.soc_max:
#             # Charging from PV surplus
#             # if fc > 0:
#             #     flow2b = min(load - pv + fc, self.max_p)
#             # else:
#             flow2b = max(load - pv - fc, self.min_p)

#             energy_in = -1*flow2b * dt_h * self.charge_eff
#             soc_increment = (energy_in / cap_kwh) * 100
#             soc = soc + soc_increment
#             bat_state = 'charging'
        
#         elif signal == 2 and soc <= self.soc_max:
#             # Charging from grid surplus
#             flow2b = max(load - pv - fc , self.min_p)
#             energy_in = -1*flow2b * dt_h * self.charge_eff
#             soc_increment = (energy_in / cap_kwh) * 100
#             soc = soc + soc_increment
...
#         elif signal == -1 and soc >= self.soc_min:
#             # Discharging to cover load
#             flow2b = min(load - pv, self.max_p)
#             energy_out = flow2b * dt_h / self.discharge_eff
#             soc_decrement = (energy_out / cap_kwh) * 100 
#             soc = soc - soc_decrement
#             bat_state = 'discharging'

#         elif signal == -2 and soc >= self.soc_min:
#             # Discharging to support electrolyser
#             required_power = load - pv + (-1*flow2e) #fc should be zero though
#             flow2b = min(required_power, self.max_p)
#             energy_out = flow2b * dt_h / self.discharge_eff
#             soc_decrement = (energy_out / cap_kwh) * 100
#             soc = soc - soc_decrement
#             bat_state = 'discharging'

#         elif signal == -3 and soc >= self.soc_min:
#             # Discharging to support fuel cell
#             required_power = load - pv - fc
#             flow2b = min(required_power, self.max_p)
#             energy_out = flow2b * dt_h / self.discharge_eff
#             soc_decrement = (energy_out / cap_kwh) * 100
#             soc = soc - soc_decrement
